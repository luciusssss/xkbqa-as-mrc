...
from utils_squad import SquadChoiceProcessor, squad_choice_convert_examples_to_features,compute_predictions_logits_for_choice_mrc, post_process_results
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            examples = SquadChoiceProcessor.get_examples_from_dataset(tfds_examples, evaluate=evaluate)
        else:
            processor = SquadChoiceProcessor()
            if evaluate:
                examples = processor.get_dev_examples(args.data_dir, filename=args.predict_file)
            else:
                examples = processor.get_train_examples(args.data_dir, filename=args.train_file)

        features, dataset = squad_choice_convert_examples_to_features(
            examples=examples,
            tokenizer=tokenizer,
